...
function erzeugeEintragElement(eintrag) {
...
async function init() {
    const container = document.querySelector("#eintraege");
    if (!container)
        return;
    try {
        ...
        if (erstesDetails)
            erstesDetails.setAttribute("open", "");
    }
    catch (error) {
    ...
    const filterBtn = document.querySelector("#nurHuBtn");
    filterBtn === null || filterBtn === void 0 ? void 0 : filterBtn.addEventListener("click", () => {
        ...
        const alleEintraege = Array.from(container.querySelectorAll("details"));
        alleEintraege.forEach((d) => {
            const zeigen = !aktiv || d.classList.contains("hat-hü");
            d.classList.toggle("ausgeblendet", !zeigen);
        });
    });
    const toggleBtn = document.querySelector("#toggleAlleBtn");
